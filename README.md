...
## 1. Структура конфігураційних файлів

Проект керується двома основними конфігураційними файлами у форматі JSON: `config.json` для загальних налаштувань та `objects.json` для визначення об'єктів і їх зв'язку з маркерами.

### 1.1. Файл `config.json`

Цей файл містить глобальні параметри програми, що впливають на камеру, візуалізацію, логіку детектування та аудіо.
...
*   `gridProximityThreshold` (number): Поріг наближення на дошці в клітинках сітки для активації звуку.
...
### 1.2. Файл `objects.json`

Цей файл визначає список об'єктів, таких як шахові фігури, контрольні та межові маркери, і пов'язує їх з конкретними `marker_id`.
...
**Опис полів в масиві `objects`:**
*   `obj_id` (number): Унікальний числовий ідентифікатор об'єкта.
*   `marker_id` (number): Ідентифікатор ArUco-маркера, що відповідає цьому об'єкту.
*   `name` (string): Назва об'єкта (наприклад, `"біла тура"`).
*   `audio_name` (string): Назва аудіофайлу, що відтворюється при взаємодії.
...
## 2. Активні структури даних (Data Structures)

#### Клас `Marker` (модуль `js/models/Marker.js`)
...
#### Стан калібрування `Calibration` (модуль `js/core/Calibration.js`)
...
#### Стан блокування об'єкта `lockedObjectState` (модуль `js/detector/ProximityDetector.js`)
...
## 3. Неактивні структури даних (Мертвий код моделей)
