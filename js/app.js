...
console.log("global cv:",typeof window.cv);
console.log(window.cv);
console.log(
    "instanceof Promise =",
    window.cv instanceof Promise
);
console.log(Object.keys(window.cv));
console.log("Mat =", window.cv.Mat);
console.log("getBuildInformation =", window.cv.getBuildInformation);
console.log("cvtColor =", window.cv.cvtColor);
...
// Перевірка ArUco API
console.log("cv.aruco_ArucoDetector:",cv.aruco_ArucoDetector);
...
console.log("FrameConverter created");
...
async function start() {
    ...
    if (video.readyState === HTMLMediaElement.HAVE_ENOUGH_DATA) {
        ...
        console.log("Frame received:",frame.width,frame.height,frame.timestamp);
        // ------------------------------------
        // 4. Конвертація Frame -> cv.Mat
        // ------------------------------------
        console.log("1. Before convert");
        const mat = frameConverter.convert(frame);
        console.log("2. After convert");
        /*        console.log(
                    "OpenCV Mat:",
                    "rows =", mat.rows,
                    "cols =", mat.cols,
